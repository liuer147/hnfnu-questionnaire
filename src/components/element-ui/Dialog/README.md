  ...
  <div style="padding: 20px">
    ...
    <al-dialog v-model="showDialog" handle-button>
      ...
      <template slot="dialog-footer">
        <al-button
          v-if="stepActive > 0"
          v-bind="handleButton[0]"
          @click="buttonClick"
          >{{ handleButton[0].label }}</al-button
        >
        <al-button
          v-if="stepActive < 3"
          v-bind="handleButton[1]"
          @click="buttonClick"
          >{{ handleButton[1].label }}</al-button
        >
        <al-button v-bind="handleButton[2]" @click="buttonClick">{{
          handleButton[2].label
        }}</al-button>
        <al-button v-bind="handleButton[3]" @click="buttonClick">{{
          handleButton[3].label
        }}</al-button>
      </template>
    </al-dialog>
  </div>
